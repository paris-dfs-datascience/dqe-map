...
export default function DQEBattleCardMap() {
  ...
  const showBattleCardInfo = (marker: google.maps.Marker, card: any, map: google.maps.Map) => {
    ...
    const infoContent = `<div style="max-width:500px;max-height:600px;overflow-y:auto;font-family:Arial,sans-serif;">
      ...
      <details open style="margin-bottom:12px;border:1px solid #ddd;border-radius:4px;"><summary style="background:#f5f5f5;padding:10px;cursor:pointer;font-weight:bold;font-size:14px;">DQE Network Data</summary><div style="padding:12px;font-size:12px;">${cbData.DQE_Site_Distance&&cbData.DQE_Site_Distance!=='NOT_FOUND'&&cbData.DQE_Site_Distance!=='N/A'?`<table style="width:100%;margin-bottom:8px;"><tr><td style="padding:4px;font-weight:bold;width:50%;">Distance:</td><td style="padding:4px;">${cbData.DQE_Site_Distance} feet</td></tr><tr><td style="padding:4px;font-weight:bold;">Connection Status:</td><td style="padding:4px;">${cbData.DQE_Connection_Status||'N/A'}</td></tr><tr><td style="padding:4px;font-weight:bold;">Network Status:</td><td style="padding:4px;">${cbData.DQE_Network_Status||'N/A'}</td></tr><tr><td style="padding:4px;font-weight:bold;">Access Medium:</td><td style="padding:4px;">${cbData.DQE_Access_Medium||'N/A'}</td></tr></table>${cbData.SITE_All_Competitors&&cbData.SITE_All_Competitors!=='N/A'?`<div style="margin-top:12px;"><strong style="font-size:11px;">Competitors at Site:</strong><div style="margin-top:4px;padding:8px;background:#FFF3E0;border-radius:4px;font-size:11px;">${cbData.SITE_All_Competitors}</div></div>`:''}`:`<p style="color:#999;font-style:italic;">No DQE network data available</p>`}</div></details>
