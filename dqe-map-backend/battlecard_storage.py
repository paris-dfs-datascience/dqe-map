...
class BattleCardStorage:
    ...
    def _calculate_summary(self, battle_cards: List[Dict],
        ...
        top_prospects = sorted(
            [
                {
                    "business_name": bc['ey_file_data'].get('Name', 'Unknown'),
                    "address": bc['ey_file_data'].get('Address', ''),
                    "city": bc['ey_file_data'].get('City', ''),
                    "state": bc['ey_file_data'].get('State', ''),
    ...
    def save_to_gcs(self, battle_cards: List[Dict], output_name: str,
        ...
        try:
            blob_path = f"{BATTLECARD_OUTPUT_PREFIX}/{output_name}.json"
